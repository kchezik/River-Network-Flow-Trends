...
library(nlme)
mod = gls(slope~s1 + s2, data = x$Area$real_slopes, weights = varComb(varExp(form = ~s1), varExp(form = ~s2)))
mod = gls(slope~s1 + s2, data = x$Area$real_slopes, weights = varExp(form = ~s2))
